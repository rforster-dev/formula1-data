  ...
  <div class="container px-5 py-24 mx-auto">
    <div class="flex flex-wrap w-full mb-20">
      <div class="lg:w-1/2 w-full mb-6 lg:mb-0">
        <h1 class="sm:text-3xl text-2xl font-medium title-font mb-2 text-gray-900">All {{totalRaces}} races decided it in the {{seasonYear}} season</h1>
        <div class="h-1 w-20 bg-indigo-500 rounded"></div>
        
      </div>
      <p class="lg:w-1/2 w-full leading-relaxed text-gray-500">It wasn't all plain sailing for all the drivers, it turned out that we had {{countClassifications}} different classifications in the 2021 season. Ranging from; <span class="inline" v-for="classification in allClassifications.splice(0,5)" :key="classification.key">{{classification.status}} ({{classification.count}}), </span><a class="text-indigo-500 inline-flex items-center mt-4" href="/all-drivers.html">Driver classifications throughout the season <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-4 h-4 ml-2" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"></path></svg></a></p>
     
    </div> 
    <div class="flex flex-wrap -m-4">
      <div v-for="race in firstThreeRaces" :key="race.key" class="xl:w-1/4 md:w-1/2 p-4">
